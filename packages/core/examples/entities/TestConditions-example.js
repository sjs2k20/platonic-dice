...
// Class instance
const classConditions = new TestConditions(TestType.AtLeast, 15, DieType.D20);
...
const atLeast = new TestConditions(TestType.AtLeast, 15, DieType.D20);
console.log(`AtLeast: target >= ${atLeast.target}`);

const atMost = new TestConditions(TestType.AtMost, 5, DieType.D20);
console.log(`AtMost: target <= ${atMost.target}`);

const exact = new TestConditions(TestType.Exact, 10, DieType.D20);
console.log(`Exact: target === ${exact.target}`);
...
const difficulties = {
  trivial: new TestConditions(TestType.AtLeast, 5, DieType.D20),
...
Object.entries(difficulties).forEach(([name, conditions]) => {
  console.log(`${name}: DC ${conditions.target}`);
});
console.log();
...
console.log(`Level 1: DC ${level1Check.target} (from base DC 18)`);
console.log(`Level 5: DC ${level5Check.target} (from base DC 18)`);
console.log(`Level 10: DC ${level10Check.target} (from base DC 18)\n`);
...
const skills = {
  athletics: new TestConditions(TestType.Skill, 15, DieType.D20),
  acrobatics: new TestConditions(TestType.Skill, 12, DieType.D20),
  perception: new TestConditions(TestType.Skill, 10, DieType.D20),
  stealth: new TestConditions(TestType.Skill, 18, DieType.D20),
};

Object.entries(skills).forEach(([skillName, conditions]) => {
  const check = rollTest(DieType.D20, conditions);
  console.log(
    `${skillName} (DC ${conditions.target}): ${check.roll} → ${check.outcome}`
  );
});
console.log();
...
const d4Check = new TestConditions(TestType.AtLeast, 3, DieType.D4);
const d6Check = new TestConditions(TestType.AtLeast, 4, DieType.D6);
const d20Check = new TestConditions(TestType.AtLeast, 15, DieType.D20);
...
console.log(`  Created successfully: DC ${valid.target}\n`);
...
const complete = new TestConditions(TestType.Skill, 14, DieType.D20);

console.log("Properties:");
console.log(`  testType: ${complete.testType}`);
console.log(`  target: ${complete.target}`);
